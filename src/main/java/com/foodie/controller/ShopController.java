...
public class ShopController {
	...
	public ResponseEntity<?> updateShopProfile(@PathVariable Long id, @RequestBody Shop updatedDetails) {
		...
		if (shopOpt.isPresent()) {
			...
			if (updatedDetails.getShopName() != null)
				shop.setShopName(updatedDetails.getShopName());
			if (updatedDetails.getOwnerName() != null)
				shop.setOwnerName(updatedDetails.getOwnerName());
			if (updatedDetails.getMobile() != null)
				shop.setMobile(updatedDetails.getMobile());
			if (updatedDetails.getCategory() != null)
				shop.setCategory(updatedDetails.getCategory());
			if (updatedDetails.getAddress() != null)
				shop.setAddress(updatedDetails.getAddress());
			if (updatedDetails.getFssaiLicense() != null)
				shop.setFssaiLicense(updatedDetails.getFssaiLicense());
			if (updatedDetails.getImageUrl() != null)
				shop.setImageUrl(updatedDetails.getImageUrl());
	...
	public ResponseEntity<?> adminToggleShopStatus(@PathVariable Long id, @RequestParam boolean active) {
		...
		if (shopOpt.isPresent()) {
			...
			return ResponseEntity
					.ok(java.util.Map.of("status", "success", "message", "Shop active status updated to: " + active));
		}
		return ResponseEntity.status(404).body("Shop not found");
	}
}